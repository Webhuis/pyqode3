...
class GlobalCheckerPanel(Panel):
    ...
    def _draw_messages(self, painter):
        ...
        for checker_mode in checker_modes:
            for msg in checker_mode.messages:
                ...
                rect = QtCore.QRect()
                rect_setX_int=int((self.sizeHint().width() / 4))
                rect.setX(rect_setX_int)
    ...
    def get_marker_height(self):
        ...
        """
        return int(self.editor.viewport().height() / TextHelper( self.editor).line_count())
    ...
    def get_marker_size(self):
        ...
        if h < 1:
            h = 1
        return QtCore.QSize(int(self.sizeHint().width() / 2), h)
